...
for (itemnum in simpleJSON) {
    // console.log(JSON.stringify(simpleJSON[itemnum], null, 4));
    var cuisine = simpleJSON[itemnum]['cuisine'].toLowerCase();
    var funfact = simpleJSON[itemnum]['fun facts'];

    if (! recipenuggets.hasOwnProperty(cuisine)) {
        recipenuggets[cuisine] = [];
    }

    recipenuggets[cuisine].push(funfact);

}

// console.log(JSON.stringify(recipenuggets, null, 4));

var dict = {recipenuggets: recipenuggets};
